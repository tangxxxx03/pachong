...
- 关键词：默认 ["外包","派遣"]，可自行扩展
- 抽取：金额 / 联系人 / 电话 / 简要摘要 / （新增）投标截止
- 中标：优先解析表格（供应商/报价/得分），否则文本兜底
- PDF 公告：自动下载并用 pdfplumber 抽取文本，再识别投标截止等字段
- 输出：直接以 Markdown 文本分片推送到钉钉机器人
"""
...
def _pick(text, pat):
    g = re.search(pat, text, re.S|re.I); return g.group(1).strip() if g else "暂无"
...
def _normalize_date_string(s: str) -> str:
    """把 '2025年8月14日 17:00' / '2025/8/14 17:00' / '2025-08-14' 统一为 'YYYY-MM-DD HH:MM' 或 'YYYY-MM-DD'"""
    if not s: return ""
...
def parse_award_from_tables(html: str):
    ...
    for tb in tables:
        ...
        joined = "".join(cols)
        if not any(k in joined for k in ["供应商名称","供应商","单位名称"]): continue
        def find_col(keys):
            for k in keys:
                for c in cols:
                    if k in c: return c
