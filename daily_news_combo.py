# -*- coding: utf-8 -*-
"""
三茅网 + 财富中文网 合并爬虫 V14 (完全体：修复乱码 + 链接修复 + AI 总结)
—————————————————————————
功能：
1）从三茅网抓取「三茅日报」要点
2）从财富中文网抓取文章 -> 抓取正文 -> 调用 AI 生成一句话总结
3）合成 Markdown 推送到钉钉

环境变量（GitHub Secrets）：
  DINGTALK_BASES   = https://oapi.dingtalk.com/robot/send?access_token=xxx
  DINGTALK_SECRETS = xxx

  # AI 配置 (SiliconFlow / OpenAI)
  OPENAI_API_KEY   = sk-xxxx
  AI_API_BASE      = https://api.siliconflow.cn/v1 (可选)
  AI_MODEL         = Qwen/Qwen2.5-7B-Instruct (可选)
...
try:
    ...
    print("⚠️ 缺少 openai 库，将跳过 AI 总结 (pip install openai)")
    HAS_OPENAI = False
# -----------------------
...
def get_ai_summary(content: str, title: str = "") -> str:
    ...
    try:
        resp = AI_CLIENT.chat.completions.create(
            model=AI_MODEL,
            messages=[
                {"role": "system", "content": "你是一个严谨的新闻编辑。请将新闻正文概括为一句中文陈述句，要求客观、包含核心信息，长度不超过50字。"},
...
class HRLooCrawler:
    def __init__(self):
        ...
        self.results = []
        self.target_date = now_tz().date() # 默认今天
        # 如果配置了 HR_TARGET_DATE 则覆盖
    ...
    def _fetch_detail(self, url):
        try:
            ...
            for st in container.select("strong"):
                t = norm(st.get_text())
                # 过滤掉"阅读量"等无关信息，提取序号后的内容
                t = re.sub(r"^\d+[.、]\s*", "", t)
                if len(t) > 5 and "阅读" not in t:
                    titles.append(t)
            
            # 如果没找到 strong，尝试 p 标签带序号的
            if not titles:
...
class FortuneCrawler:
    ...
    def run(self):
        ...
        try:
            ...
            r.encoding = "utf-8" # 强制 UTF-8 修复乱码
            soup = BeautifulSoup(r.text, "html.parser")
            ...
            for li in soup.select("ul.news-list li.news-item"):
                ...
                href = a["href"].strip()
                if "content_" not in href: continue # 过滤非文章
...
def build_fortune_md(crawler):
    ...
    for i, it in enumerate(crawler.items, 1):
        # 使用 AI 总结作为显示文本，如果没总结则用原标题
        display_text = it['summary'] if it['summary'] else it['title']
