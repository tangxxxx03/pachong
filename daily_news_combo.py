# -*- coding: utf-8 -*-
"""
三茅网 + 财富中文网 合并爬虫 V19 (深度逻辑版：讲清前因后果)
—————————————————————————
核心更新：
1. 扩容字数：从 50 字放宽到 100 字，给 AI 解释“为什么”的空间。
2. 逻辑链 Prompt：强制要求包含 [背景/起因] -> [冲突/动作] -> [结局/影响]。
3. 保持人话：依然拒绝翻译腔，但现在更像“深度点评”而非“快讯标题”。
"""
...
def get_ai_summary(content: str, title: str = "") -> str:
    ...
    # --- 🧠 深度逻辑 Prompt 设计 ---
    system_prompt = (
        "你是一个不仅看结果，更看重逻辑链条的**深度商业分析师**。你的任务是通俗易懂地解释新闻背后的逻辑。\n\n"
        "**撰写要求：**\n"
        "1. **讲清前因后果**：不要只扔一个结论！要解释清楚**“为什么会这样？”**（背景/原因）以及**“这意味着什么？”**（结果/影响）。\n"
        "2. **结构建议**：采用 `背景/冲突 -> 核心事件/观点 -> 最终结局` 的逻辑流。\n"
        "3. **拒绝复读**：禁止每条都以“某人表示”、“某公司宣布”开头，尝试把主语后置或隐藏，突出事件本身。\n"
        "4. **人话表达**：用词要犀利、有洞察力，避免像政府公文一样枯燥。\n"
        "5. **长度限制**：中文 **80-100字** 左右（给足空间讲逻辑）。"
    )
    ...
    try:
        resp = AI_CLIENT.chat.completions.create(
            ...
            max_tokens=180, # 增加 token 上限，允许 AI 多说点
            temperature=0.3 # 稍微冷静一点，保证逻辑严密
        )
