...
依赖：requests beautifulsoup4 urllib3（与之前一致）
"""
...
def parse_dt(text: str) -> Optional[datetime]:
    ...
    for pat in DATE_PATS:
        ...
        if len(m.groups())==3:
            ...
            return datetime(y,mo,d,12,0,tzinfo=TZ)   # 只有日期 → 默认中午，避免被窗口误杀
        if len(m.groups())==4:
...
def auto_range(days_for_monday: int = 3) -> Tuple[datetime, datetime, str]:
    ...
    now = datetime.now(TZ)
    if now.weekday() == 0:  # 周一
        end = datetime(now.year, now.month, now.day, 23,59,59, tzinfo=TZ) - timedelta(days=1)  # 昨天的 23:59:59
